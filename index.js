...
const backersSum = GAMES_JSON.reduce((sum, item) => sum + item.backers, 0);

// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = `${backersSum.toLocaleString()}`;

// grab the amount raised card, then use reduce() to find the total amount raised
const raisedCard = document.getElementById("total-raised");
const raisedSum = GAMES_JSON.reduce((sum, item) => sum + item.pledged, 0);

// set inner HTML using template literal
raisedCard.innerHTML = `$${raisedSum.toLocaleString()}`;
...
const unfundedString =
  unfundedGames.length === 1
    ? `A total of $${raisedSum.toLocaleString()} has been raised for ${
        GAMES_JSON.length
